...
class Properties(object):
    ...
    def load(self, propFile):
        """ Load properties from an open properties file  """
        try:
    ...
    def store(self, out, header=""):
        ...
        try:
            ...
            line = None
            try:
                tstamp = time.strftime('%d.%m.%Y %H:%M:%S', time.localtime())
                line = ''.join(('#',tstamp,'\n'))
                out.write(line)
                ...
                logging.exception(e)
                logging.error("Failed to write to file(UTF-8):\n%s\n"%(line.encode(encoding='utf_8')))
